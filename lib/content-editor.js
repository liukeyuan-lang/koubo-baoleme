...
async function runContentEditor({ llm, currentScript, input, provider = "deepseek", mode = "revision" }) {
  ...
  const rawDiagnosis = await llm([{ role: "system", content: diagnosisSystem }, { role: "user", content: JSON.stringify(diagnosisPayload) }], 0.05, { timeoutMs: 35000, maxTokens: 2600, provider: "deepseek" });
  ...
  const rawRevision = await llm([{ role: "system", content: revisionSystem }, { role: "user", content: JSON.stringify({ currentScript: previousScript, evidence, confirmedUserConclusion: input.confirmedUserConclusion || "", editContract, editorialDiagnosis: diagnosis, creatorVoiceSummary }) }], 0.05, { timeoutMs: 35000, maxTokens: 2800, provider });
  ...
  if (!passed) {
    ...
    candidate = makeCandidate(repairedRaw); realDiff = buildRealDiff(previousScript, candidate); verifiedAppliedActions = verifyPlan(diagnosis.editorialPlan, realDiff, editContract); deterministicTasteIssues = mechanicalIssues(candidate); editorCallCount = 5;
    const deterministicPlanPassed = verifiedAppliedActions.every((item) => item.executed);
    const rejectedClaims = [...(initialValidation.factPreservation?.addedUnsupportedFacts || []), ...(initialValidation.factPreservation?.changedFacts || [])].map((item) => compactText(typeof item === "string" ? item : item.claim || item.text || "")).filter(Boolean);
    const repairedText = compactText(scriptText(candidate)); const factsResolved = rejectedClaims.every((claim) => !repairedText.includes(claim));
    const changedEnough = realDiff.summary.changedSections > 0;
    const conclusionPassed = !editContract.preserve.includes("conclusion") || initialValidation.conclusionPreservation?.status === "PASS";
    const speakabilityPassed = initialValidation.speakability?.status === "PASS";
    const factPassed = factsResolved;
    const planPassed = deterministicPlanPassed && changedEnough;
    const tastePassed = deterministicTasteIssues.length === 0;
    passed = factPassed && conclusionPassed && planPassed && tastePassed && speakabilityPassed;
    finalValidation = {
      status: passed ? "PASS" : "FAIL",
      phase: "FINAL_AFTER_TARGETED_REVISION",
      factPreservation: { ...(initialValidation.factPreservation || {}), status: factPassed ? "PASS" : "FAIL", addedUnsupportedFacts: factsResolved ? [] : (initialValidation.factPreservation?.addedUnsupportedFacts || []), removedImportantFacts: factPassed ? [] : (initialValidation.factPreservation?.removedImportantFacts || []), changedFacts: factsResolved ? [] : (initialValidation.factPreservation?.changedFacts || []) },
      conclusionPreservation: { ...(initialValidation.conclusionPreservation || {}), status: conclusionPassed ? "PASS" : "FAIL" },
      planExecution: { status: planPassed ? "PASS" : "FAIL", missedActions: verifiedAppliedActions.filter((item) => !item.executed).map((item) => `${item.action}:${item.target}`), unplannedChanges: [] },
      tasteCheck: { status: tastePassed ? "PASS" : "FAIL", issues: deterministicTasteIssues },
      speakability: { ...(initialValidation.speakability || {}), status: speakabilityPassed ? "PASS" : "FAIL" },
      targetedRevision: { attempted: true, deterministicPlanPassed, deterministicTasteIssues, changedEnough, factsResolved },
    };
  }
