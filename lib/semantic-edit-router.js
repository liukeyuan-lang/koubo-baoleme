...
判断expansionMode时必须理解整句语义，不得根据“丰满、扩写、展开、扩充”等关键词硬匹配。EXPRESSIVE表示用户只希望把已有事实、关系和判断讲得更完整、更有层次，不要求新案例或新信息；FACTUAL表示用户确实要增加当前Evidence中没有的案例、数据、经历、过程、原因或结果；NONE表示本次不是展开请求。EXPRESSIVE必须needsEvidence=false，并允许length、wording、transition、explanation；FACTUAL必须needsEvidence=true。

只返回JSON：{"userGoal":"","problems":[],"targets":[],"feedbackLevels":["EXPRESSION|STRUCTURE|EVIDENCE|DIRECTION"],"primaryFeedbackLevel":"EXPRESSION|STRUCTURE|EVIDENCE|DIRECTION","rejectsCurrentDirection":false,"rollbackTo":"SCRIPT|GLOBAL_DIAGNOSIS|EVIDENCE_CHECK|CONTENT_DIRECTION","diagnosisMode":"STANDARD|ROOT_CAUSE_DISCOVERY","scope":"LOCAL|CROSS_SECTION|GLOBAL","changeMagnitude":"LOW|MEDIUM|HIGH","expansionMode":"NONE|EXPRESSIVE|FACTUAL","preserve":[],"allowedChanges":[],"forbiddenChanges":[],"needsEvidence":false,"evidenceKind":"PERSONAL|PUBLIC|MIXED|NONE","needsResearch":false,"reasoningSummary":""}。`;
...
function normalizeEditContract(raw = {}) {
  ...
  const expansionMode = ["NONE", "EXPRESSIVE", "FACTUAL"].includes(raw.expansionMode) ? raw.expansionMode : "NONE";
  const needsEvidence = expansionMode === "EXPRESSIVE" ? false : expansionMode === "FACTUAL" ? true : raw.needsEvidence === true;
  ...
  if (expansionMode === "EXPRESSIVE") allowedChanges = strings([...allowedChanges, "length", "wording", "transition", "explanation"]);
  return {
    ...
    expansionMode, preserve, allowedChanges, forbiddenChanges: strings(raw.forbiddenChanges), needsEvidence, evidenceKind, needsResearch: needsEvidence && ["PUBLIC", "MIXED"].includes(evidenceKind) && raw.needsResearch === true,
